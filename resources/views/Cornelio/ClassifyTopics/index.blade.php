                ...
                <div class="card">
                    <form action="{{ route('ClassifyTopics.index') }}" method="get">
                        <div class="card-header">
                            <h4>Clasificar tema para Facebook
                                @can('ClassifyTopics.classify')
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                @foreach ($classifications as $classification)
                                    <tr>
                                        ...
                                        <td>{{ $classification['page']->page_name}}</td>


                                        @if(isset($classification['post']))
                                        <td><div class="cortar">{{ $classification['post']->content }}</div></td>

                                        @else
                                            <td>{{ ' '}}</td>
                                        @endif

                                        @if(isset($classification['subcategory']))
                                        <td>{{ $classification['subcategory']->name}}</td>
                                        @else
                                            <td>{{ ' '}}</td>
                                        @endif
                                        <td>
...
@section('script')
    <script>
        ...
        function ModalCrear() {
            ...
            axios.post('{{ route('ClassifyTopics.classify') }}', data).then(response => {
                //window.location = "{{ route('ClassifyTopics.index') }}";
            }).catch(error=>{
        ...
        function show(id){
            ...
            axios.post("{{route('ClassifyTopics.edit')}}",data).then( response => {
        ...
        function ModalEditar() {
            ...
            axios.post('{{ route('ClassifyTopics.update') }}', data).then(response => {
                window.location = "{{ route('ClassifyTopics.index') }}";
            }).catch(error=>{
