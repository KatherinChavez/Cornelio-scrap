...
class BublesContentD extends Command
{
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:BublesContentD';
     ...
     * @var string
     */
    protected $description = 'Burbuja con las palabras de los contenidos al iniciar el dia';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->whereIn('report', [1,5,9])->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //$numbers = NumberWhatsapp::where('report', 13)->get();
        $numbers = NumberWhatsapp::where('report', 13)->where('company_id', '!=',24)->get();
