...
class TopComparatorT extends Command
{
    //use TopicCountTrait;
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:TopComparatorT';
     ...
     * @var string
     */
    protected $description = 'Analiza el top 5 del pdf de las reacciones en la tarde';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->whereIn('report', [3,7])->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //Numero de telefono
        $numbers = NumberWhatsapp::where('report', 11)->get();
