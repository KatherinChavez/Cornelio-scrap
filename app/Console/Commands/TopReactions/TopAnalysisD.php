...
class TopAnalysisD extends Command
{
    //use TopicCountTrait;
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:TopAnalysisD';
     ...
     * @var string
     */
    protected $description = 'Analisa los top reacciones y se envia deacuerdo a la empresa en la mañana ';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->where('report', 1)->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //Numero de telefono
        $numbers = NumberWhatsapp::where('report', 5)->get();
