...
class BublesContentN extends Command
{
    //use TopicCountTrait;
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:BublesContentN';
     ...
     * @var string
     */
    protected $description = 'Burbuja con las palabras de los contenidos al finalizar el dia';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->whereIn('report', [4,8,12])->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //$numbers = NumberWhatsapp::where('report', 16)->get();
        $numbers = NumberWhatsapp::where('report', 16)->where('company_id', '!=',24)->get();
