...
class BublesContentT extends Command
{
    //use TopicCountTrait;
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:BublesContentT';
     ...
     * @var string
     */
    protected $description = 'Burbuja con las palabras de los contenidos en la tarde';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->whereIn('report', [3,7,11])->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //$numbers = NumberWhatsapp::where('report', 15)->get();
        $numbers = NumberWhatsapp::where('report', 15)->where('company_id', '!=',24)->get();
