...
class TopComparatorN extends Command
{
    //use TopicCountTrait;
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:TopComparatorN';
     ...
     * @var string
     */
    protected $description = 'Analiza el top 5 del pdf de las reacciones de la noche';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->whereIn('report', [4,8])->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //Numero de telefono
        $numbers = NumberWhatsapp::where('report', 12)->get();
