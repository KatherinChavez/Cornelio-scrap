...
class ResendDay extends Command
{
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:ResendDay';
     ...
     * @var string
     */
    protected $description = 'Se reenvia los mensajes que no han salido en mañana';
    ...
    public function handle()
    {
         ...
         * 13 BURBUJA DE PALABRAS DE LAS 6:40 */
    ...
    public function resendTopReaction(){
        $dataPdf = BitacoraMessageFb::where('report', 1)->orderBy('report', 'ASC')->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->get();
        ...
        //ENVIO DE ANALISIS
        $data = BitacoraMessageFb::where('report', 1)->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->groupBy('number')->get();
    ...
    public function resendTopAnalysis(){
        $dataAnalysis = BitacoraMessageFb::where('report', 5)->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->orderBy('report', 'ASC')->get();
        foreach ($dataAnalysis as $sendAnalysis){
            $this->sendTopAnalysis($sendAnalysis);
        }
        sleep(10);

        $data = BitacoraMessageFb::where('report', 5)->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->groupBy('number')->get();
        //ENVIO DE COMPARADOR
        foreach ($data as $sendComparator){
            $report  = 9;
            $numbers = $sendComparator->type == 'phone' ?
    ...
    public function resendTopComparator(){
        $dataComparator = BitacoraMessageFb::where('report', 9)->orderBy('report', 'ASC')->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->get();
        foreach ($dataComparator as $sendComparator){
            $this->sendTopComparator($sendComparator);
        }
        sleep(10);

        $data = BitacoraMessageFb::where('report', 9)->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->groupBy('number')->get();
        //ENVIO DE BURBUJA DE PALABRA
        foreach ($data as $sendBuble){
            $report  = 13;
            $numbers = $sendBuble->type == 'phone' ?
    ...
    public function resendTopBubles(){
        $dataBubles = BitacoraMessageFb::where('report', 13)->orderBy('report', 'ASC')->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->get();
