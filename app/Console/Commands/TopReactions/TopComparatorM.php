...
class TopComparatorM extends Command
{
    //use TopicCountTrait;
    use SendReaction;
    /**
     ...
     * @var string
     */
    protected $signature = 'Cornelio:TopComparatorM';
     ...
     * @var string
     */
    protected $description = 'Analiza el top 5 del pdf de las reacciones al medio dia';
    ...
    public function handle()
    {
        ...
        /*foreach ($numbers as $number){
            ...
            $query  = BitacoraMessageFb::where('number', $id)->whereIn('report', [2,6])->whereBetween('created_at',[Carbon::today()->format("Y-m-d 00:00:00"), Carbon::now()])->first();
            if(!$query){
                $send   = [];
                $send[] = $number;
        ...
        //Ultimos minutos
        $last = Carbon::now()->subMinute(40);
        //$last = Carbon::now()->subHour(6);
        //Numero de telefono
        $numbers = NumberWhatsapp::where('report', 10)->get();
